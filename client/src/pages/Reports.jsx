...
export default function Reports() {
  ...
  function downloadPdf() {
    const doc = new jsPDF();
    const gradeRows = uniqueGradeRows(report.grades);
    ...
    gradeRows.forEach((g, i) => doc.text(`${g.subject_name} - ${g.term}: ${g.average} ${g.remarks}`, 15, 58 + i * 8));
    doc.save(`${report.student.name}-SmartBuddy-report.pdf`);
  }
  const gradeRows = report ? uniqueGradeRows(report.grades) : [];
  ...
  return <div>
    ...
    {!report ? <EmptyState title="No report selected" text="Select a student and generate a report." /> : <div ref={ref} className="card p-8">
      ...
      <h2 className="mt-6 font-bold">Grades</h2><table className="mt-2 w-full text-left text-sm"><tbody>{gradeRows.map((g) => <tr className="border-b" key={`${g.subject_id}-${g.term}`}><td className="py-2">{g.subject_name}</td><td>{g.term}</td><td>{g.average}</td><td>{statusBadge(g.remarks)}</td></tr>)}</tbody></table>
...
function uniqueGradeRows(rows) {
  const latest = new Map();
  for (const row of rows || []) {
    latest.set(`${row.subject_id}-${row.term}`, row);
  }
  return [...latest.values()].sort((a, b) => `${a.subject_name}${a.term}`.localeCompare(`${b.subject_name}${b.term}`));
}
